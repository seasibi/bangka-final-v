...
const AdminReportGeneration = () => {
  ...
  // Process rows -> processedData with filter/group/sort UI parity with Municipal
  useEffect(() => {
  ...
  const handleGenerate = () => {
    ...
    // Signatories (Admin + Provincial Agriculturist)
    try {
      const pw = doc.internal.pageSize.getWidth();
      ...
      const adminName = (user ? `${user.first_name || ''} ${user.middle_name ? user.middle_name.charAt(0)+'. ' : ''}${user.last_name || ''}` : '').replace(/\s+/g,' ').trim().toUpperCase();
      const adminRole = (user?.user_role || '').replace(/_/g,' ').replace(/\b\w/g, c => c.toUpperCase());
      const adminTitle = `${adminRole}${user?.position ? ` - ${user.position}` : ''}` || 'Prepared by';
      ...
      drawSig(x0, adminTitle, adminName);
  ...
  const removeGroupBy = (index) => { if (groupBy.length > 1) setGroupBy(groupBy.filter((_,i)=>i!==index)); };
          ...
          <div className="lg:col-span-1">
            <div className="bg-gray-100 rounded-lg shadow-sm p-6">
              ...
              <div className="mb-6">
                ...
                <select value={reportType} onChange={(e)=>{ setReportType(e.target.value); setGroupBy(['Municipality']); setFilterBy(''); }} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={addGroupBy} disabled={groupBy.length>=3} className="p-1 text-blue-600 hover:bg-blue-50 rounded disabled:opacity-50" title="Add grouping">
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 5v14m7-7H5"/></svg>
                    </button>
                    <button onClick={()=>removeGroupBy(groupBy.length-1)} disabled={groupBy.length<=1} className="p-1 text-red-600 hover:bg-red-50 rounded disabled:opacity-50" title="Remove grouping">
                ...
                <div className="space-y-2">
                  {groupBy.map((group, idx)=> (
                    <select key={idx} value={group} onChange={(e)=>{ const ng=[...groupBy]; ng[idx]=e.target.value; setGroupBy(ng); }} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
              ...
              <div className="mb-6">
                ...
                <select value={sortBy} onChange={(e)=>setSortBy(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
              ...
              {reportType === 'Boundary Violation Report' && (
                <div className="mb-6">
                  ...
                  <select value={filterBy} onChange={(e)=>setFilterBy(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
              ...
              <button onClick={handleGenerate} disabled={loading || processedData.length===0} className="w-full py-3 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                ...
                <>
                  ...
                  {processedData.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                          ...
                          {processedData.slice(0,10).map((item,idx)=> (
                            <tr key={idx} className="border-b hover:bg-gray-50">
                              ...
                              {reportType === 'Boat Registry Report' && (<>
                                ...
                                <td className="px-4 py-2"><span className={`px-2 py-1 rounded text-xs ${item.tracker ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{item.tracker ? 'Assigned' : 'Unassigned'}</span></td>
                              </>)}
                              {reportType === 'Boundary Violation Report' && (<>
                                <td className="px-4 py-2">{item.mfbr_number || 'N/A'}</td>
                                <td className="px-4 py-2">{item.boat_name || 'N/A'}</td>
                                ...
                                <td className="px-4 py-2">
                                  <span className={`px-2 py-1 rounded text-xs ${item.report_status === 'Fisherfolk Reported' ? 'bg-green-100 text-green-800' : item.report_status === 'Resolved' ? 'bg-gray-100 text-gray-800' : 'bg-yellow-100 text-yellow-800'}`}>
                      ...
                      {processedData.length > 10 && (<p className="text-sm text-gray-500 mt-4 text-center">Showing 10 of {processedData.length} records. Click "Generate Report" to see all records.</p>)}
                    </div>
                  )}
