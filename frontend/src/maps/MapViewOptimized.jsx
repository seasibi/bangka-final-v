...
// Enhanced GPS interpolation and smoothing utilities
...
// Smooth interpolation hook for boat positions
const useSmoothInterpolation = (targetPosition, duration = 3000) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const interpolated = interpolatePosition(startPosition, targetPosition, easedProgress);
  ...
  }, [targetPosition?.lat, targetPosition?.lng, duration, currentPosition]);

  return currentPosition;
};

// Enhanced Boat Marker Component with smooth movement
const SmoothBoatMarker = ({ feature, getBoatColor, createBoatIcon }) => {
  ...
  const currentPosition = useSmoothInterpolation(targetPosition, 2000);
  if (!hasValid) {
    console.warn('Skipping marker with invalid coordinates', coords, feature?.properties);
    return null;
  }
  
  const { boat_id, mfbr_number, timestamp, status, age_seconds, identifier_icon } = feature.properties;

  // Prefer backend-provided marker_color (already aligned to municipality/tracker)
  const idForColor = mfbr_number || boat_id;
  const baseColor = feature?.properties?.marker_color || getBoatColor(idForColor);
  const iconType = identifier_icon || 'boat';
  const icon = createBoatIcon(baseColor, status, iconType);
...
// WebSocket Hook for Real-time GPS Updates
...
const MapViewOptimized = () => {
  ...
  // Process GPS data from WebSocket with deadband filtering
  useEffect(() => {
...
const status = age != null && age > 300 ? 'offline' : 'online';
  ...
  useEffect(() => {
    if (connectionStatus === 'connected') return; // Don't poll if WebSocket is working
...
const status = age != null && age > 300 ? 'offline' : 'online';
    ...
    fetchBoats();
    const interval = setInterval(fetchBoats, 5000); // Reduced to 5 seconds
  ...
  const getBoatColor = useMemo(() => (id) => {
    const key = String(id ?? '');
    // Try to map by municipality if we know it (from API boat list)
    // For this optimized variant we only have ID; use stable hue as last resort
    const muni = null; // no preloaded map here
    const normalized = normalizeMuni(muni);
    if (normalized && municipalityColors[normalized]) return municipalityColors[normalized];
    let hash = 0;
  ...
  const createBoatIcon = useMemo(() => (color, status, iconType = 'boat') =>
    L.divIcon({
      className: "",
      html: iconType === 'circle'
        ? `<div class="boat-marker boat-marker-circle ${status === "offline" ? "offline" : ""}">
            <div class="marker-inner" style="background-color: ${color};"></div>
          </div>`
        : iconType === 'triangle'
        ? `<div class="boat-marker boat-marker-triangle ${status === "offline" ? "offline" : ""}">
            <svg viewBox="0 0 24 24" fill="${color}" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L2 20h20L12 2z" />
            </svg>
          </div>`
        : `<div class="boat-marker ${status === "offline" ? "offline" : ""}">
            <svg viewBox="0 0 24 24" fill="${color}" xmlns="http://www.w3.org/2000/svg">
    ...
    <>
      ...
      <div className="absolute bottom-20 right-4 bg-white/95 p-4 rounded-xl shadow-lg z-20">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-semibold">Municipalities</h3>
        </div>
